...
​	“PantherTank5.0”以ATMEGA328P-PU为主控芯片，H450作为电机驱动芯片的一款多功能履带式车，与传统小车相比，“PantherTank5.0”还搭载无线控制（蓝牙、红外遥控）等。可自动避障，当然，Maker也可通过自己的idea进行增减其他功能，如添加自动循迹、PS2游戏手柄、添加wifi控制，机械臂等。

![tank](PantherTank_pic_zh/tank.png)
...
​	在“PantherTank5.0”，我们使用了BLE-UNO作为主控板， 它有14个数字输入/输出引脚（其中6个可用作PWM输出）、6个模拟输入、1个16 MHz陶瓷谐振器、1个USB连接、1个电源插座、1个ICSP头和1个复位按钮。它包含了支持微控制器所需的一切；只需通过USB电缆将其连至计算机或者通过AC-DC适配器或电池为其供电即可开始。除此之外，还板载了CC2540蓝牙模块，可实现蓝牙通信。

![uno](PantherTank_pic_zh/uno.png)

### 扩展板MotorDriverBoard 

![mtd](PantherTank_pic_zh/mtd.png)
...
![arduino](PantherTank_pic_zh/arduino.png)

然后点击右下角选择扩展，再点击PantherTank扩展

![extensions](PantherTank_pic_zh/extensions.png)

![CHOSETANK](PantherTank_pic_zh/CHOSETANK.png

### 如何使用PantherTank图形块

选好PantherTank的扩展后就可以在左侧看到所有的PantherTank图形块了，我们就用这些图形块来为PantherTank编程

![tkblocks](PantherTank_pic_zh/tkblocks.png)
...
![tkgoforward](PantherTank_pic_zh/tkgoforward.png)
...
![upload](PantherTank_pic_zh/upload.png)
...
![motortest](PantherTank_pic_zh/motortest.png)
...
![rgbtest](PantherTank_pic_zh/rgbtest.png)
...
![buzzertest](PantherTank_pic_zh/buzzertest.png)
...
![servointr](PantherTank_pic_zh/servointr.png)

#### 示例程序

![servotest](PantherTank_pic_zh/servotest.png)
...
![1577518339169](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\1577518339169.png)

超声波避障程序

![ulavoidance](PantherTank_pic_zh/ulavoidance.png)
...
![irremote](PantherTank_pic_zh/irremote.png)
...
![irtest](PantherTank_pic_zh/irtest.png)

红外遥控操控程序

![irremotecontrol](PantherTank_pic_zh/irremotecontrol.png)
...
![ps2](PantherTank_pic_zh/ps2.png)
...
![ps2test](PantherTank_pic_zh/ps2test.png)

PS2操控程序

![ps2control](PantherTank_pic_zh/ps2control.png)
...
PantherTank主板上搭载了蓝牙，用手机的蓝牙连接，首先我们先下载emakefun 手机APP，打开APP后，选择一辆PantherTank进行蓝牙连接，我们需要编写一个蓝牙操控程序后，就可以进入APP蓝牙操控界面控制PantherTank了

#### 示例程序

![ble](PantherTank_pic_zh/ble.png)
...
![nrf](PantherTank_pic_zh/nrf.png)
