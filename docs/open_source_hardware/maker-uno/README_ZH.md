...
![1719539089912](picture/1.png)

电源有两种形式：

- 通过外部电源为 Maker Uno 板供电
- 为插入主板的一些组件供电

#### 1.2.1 为 Maker Uno 板供电
...
低电平表示该引脚上的电压为 0V。HIGH 表示 Vcc，对于 Maker Uno 来说是 5V。
...
读取时，任何施加到该引脚的电压低于 0.8V 将被视为低电平，任何大于 2V 的电压将被视为高电平。因此，我再次强调，你应该正确地将电路中的所有接地连接在一起，否则Maker Uno将无法读取有价值的信息！如果您没有获得可靠且稳定的数据，请务必先检查地面，问题很可能来自那里。
...
对于 Maker Uno，这些引脚的选择非常有限。只有数字引脚 2 和 3 可以用作中断引脚。
...
![1719539392211](picture/3.png)

您可以在 Maker Uno 板上找到 6 个模拟引脚。您会在电源引脚附近找到它们，并且它们很容易识别，从 A0 到 A5。
...
![1717660078700](picture/6.png)

Maker Uno 板有一个 10 位 ADC。如果您使用其他 Arduino 板，分辨率可能会有所不同。那么，10 位是什么意思呢？简而言之，分辨率为 2^10 = 1024。因此，从模拟输入引脚读取数据时获得的值在 0 到 1024 之间。
...
这就是事情开始变得有趣的地方。通过 Maker Uno 引脚的通信协议将允许您使用更先进的传感器和执行器。您将创建更复杂和有用的应用程序。

Maker Uno 板可通过电路引脚使用 3 种主要通信协议：UART、I2C 和 SPI。
...
当您将 Maker Uno 板连接到计算机并通过串行库进行通信时，嗯……您正在使用 UART！

您还可以直接在 Maker Uno 板上找到 UART 所需的 2 个引脚，即引脚 0 和 1：RX 和 TX。R代表“接收”，T代表“发送”。这是双向通信。

![1719539673418](picture/7.png)

请注意，USB 使用的串行与引脚 0 和 1 使用的串行相同。因此，如果您想将另一个设备连接到主板的 RX/TX 引脚，请记住不要通过 USB 使用串行。

在其他一些 Arduino 板（例如 Mega）中，有几种不同的可用 UART。但对于 Maker Uno，你只有一个。

但是，如果您想使用更多 UART，则始终可以使用SoftwareSerial 库来实现。该库允许您将任何其他数字引脚用于 UART 目的。虽然这里有一个很大的区别：“真正的”串行使用 Maker Uno 板的硬件功能，速度非常快并且不消耗太多计算能力。SoftwareSerial 则相反：它用计算能力来补偿硬件。因此，首先从标准硬件 UART 开始，然后您将看到您的应用程序是否需要更多串行端口（在这种情况下，我建议您切换到 Arduino Mega 以获得许多硬件 UART）。

要将组件连接到 Maker Uno 引脚并使用串行通信，您需要 4 根电缆：
...
通常，您将使用 Maker Uno 板作为主控板，并将一个或多个组件（通常是传感器）连接到 I2C 总线，每个组件都有不同的 ID。在软件方面，您将使用开源 Arduino Wire 库。

但是，您应该将所有这些组件连接到 Maker Uno 的哪个引脚？

对于 I2C，您无法直接在电路板上看到任何指示。

![1719539903797](picture/9.png)
...
正如您在这篇文章中看到的，在使用 Maker Uno 引脚时，您有多种选择。

如果您刚刚开始使用 Arduino，请先尝试使用数字和模拟基本功能。

然后，您可以开始进一步使用中断、PWM 输出等。

最后，您可以将无限数量的传感器/执行器连接到 Maker Uno 板来创建您的下一个项目！

您还可以将多个 Arduino 板甚至Raspberry Pi 板连接在一起。

好了，现在您已经对 Maker Uno 引脚有了更好的了解，是时候动手实践了！
