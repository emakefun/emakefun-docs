...
uint8_t WiFiEspMqtt::mqtt_connect_aliyun(const char* host, uint16_t port, const char* productKey, const char* deviceName, const char* deviceSecret, bool reconnect)
{
//  LOGDEBUG(F(">Aliyun Mqtt Connect"));

  int ret = EspDrv::sendCmd(F("AT+ALIYUN_MQTTCONN=\"%s\",%d,\"%s\",\"%s\",\"%s\""), 20000, host, 1883, productKey, deviceName, deviceSecret);
   // int ret = EspDrv::sendCmd(F("AT+ALIYUN_MQTTCONN=\"a1gVfAJo2pv.iot-as-mqtt.cn-shanghai.aliyuncs.com\",1883,\"a1gVfAJo2pv\",\"emakefun\",\"8412c9a3a13d5398fb33afc91a5f4c0c\""));
    if (ret==TAG_OK)
  {
//    LOGINFO(F("Alinyun Mqtt Connected"));
    return true;
  }

//  LOGWARN(F("Alinyun Mqtt Connect Failed"));
  delay(500);
  EspDrv::espEmptyBuf(false);
  return false;
}
...
//	LOGDEBUG(F("> Mqtt Disconnect"));
    ...
    ret = EspDrv::sendCmd(F("AT++MQTTCLEAN=0"));
	if (ret==TAG_OK)
	{
...
bool WiFiEspMqtt::ping(const char *host)
{
